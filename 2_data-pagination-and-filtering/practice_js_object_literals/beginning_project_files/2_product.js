...
function addInventory(prod, number) {
    //prod.inventory = product.inventory + number;
    prod.inventory+= number;
    console.log(`${prod.inventory} ${prod.name}s in stock`);
    console.log(`${number} ${prod.name}s added to the inventory`);
}
...
function processSale(prod, sold) {
    prod.inventory -= sold;
    let totalSale = prod.unit_price * sold;
    console.log(`${sold} ${product}s sold`);
} 
